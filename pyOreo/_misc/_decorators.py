...
from pyOreo.exceptions import DependencyMissingError
...
from ..version import oreo_version as ore_ver
...
def oreo_cmd(
    pattern=None, manager=False, oreo_bot=oreo_bot, asst=asst, **kwargs
):
    ...
    def decor(dec):
        async def wrapp(ore):
            if not ore.out:
                if owner_only:
                    return
                if ore.sender_id not in owner_and_sudos():
                    return
                if ore.sender_id in _ignore_eval:
                    return await eod(
                        ore,
                        get_string("py_d1"),
                    )
                if fullsudo and ore.sender_id not in SUDO_M.fullsudos:
                    return await eod(ore, get_string("py_d2"), time=15)
            chat = ore.chat
            if hasattr(chat, "title"):
                if (
                    "#noub" in chat.title.lower()
                    and not (chat.admin_rights or chat.creator)
                    and not (ore.sender_id in DEVLIST)
                ):
                    return
            if ore.is_private and (groups_only or admins_only):
                return await eod(ore, get_string("py_d3"))
            elif admins_only and not (chat.admin_rights or chat.creator):
                return await eod(ore, get_string("py_d5"))
            ...
            try:
                ...
                await oreo_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await oreo_bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                ...
                return await eod(ore, "`Inline Locked In This Chat.`")
            except (ChatSendMediaForbiddenError, ChatSendStickersForbiddenError):
                return await eod(ore, get_string("py_d8"))
            except (BotMethodInvalidError, UserIsBotError):
                return await eod(ore, get_string("py_d6"))
                ...
                await asst.send_message(
                    ...
                    buttons=[
                        Button.url("Bot", "t.me/StringFatherBot?start="),
                    ],
                )
                sys.exit()
                ...
                ftext += "`\n**Oreo Version:** `" + str(ore_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START OREO CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ftext += "`\n**Group:** `" + str(ore.chat_id) + "` " + str(naam)
                ftext += "\n**Sender ID:** `" + str(ore.sender_id)
                ftext += "`\n**Replied:** `" + str(ore.is_reply)
                ftext += "`\n\n**Event Trigger:**`\n"
                ftext += str(ore.text)
                ftext += "`\n\n**Traceback info:**`\n"
        ...
        if manager and MANAGER:
            ...
            async def manager_cmd(ore):
                if not allow_all and not (await admin_check(ore, require=require)):
                    return
                if not allow_pm and ore.is_private:
                    return
                try:
                    await dec(ore)
                except Exception as er:
                    if chat := udB.get_key("MANAGER_LOG"):
                        text = f"**#MANAGER_LOG\n\nChat:** `{get_display_name(ore.chat)}` `{ore.chat_id}`"
                        text += f"\n**Replied :** `{ore.is_reply}`\n**Command :** {ore.text}\n\n**Error :** `{er}`"
                        try:
                            ...
                            LOGS.exception(er)
                    LOGS.info(f"• MANAGER [{ore.chat_id}]:")
                    LOGS.exception(er)
