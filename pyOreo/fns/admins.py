...
async def _callback_check(event):
    ...
    if not _ore_cache.get("admin_callback"):
        _ore_cache.update({"admin_callback": {id_: None}})
    else:
        _ore_cache["admin_callback"].update({id_: None})
    while not _ore_cache["admin_callback"].get(id_):
        await asyncio.sleep(1)
    key = _ore_cache.get("admin_callback", {}).get(id_)
    del _ore_cache["admin_callback"][id_]
    return key


async def get_update_linked_chat(event):
    if _ore_cache.get("LINKED_CHATS") and _ore_cache["LINKED_CHATS"].get(event.chat_id):
        _ignore = _ore_cache["LINKED_CHATS"][event.chat_id]["linked_chat"]
    else:
        channel = await event.client(
            functions.channels.GetFullChannelRequest(event.chat_id)
        )
        _ignore = channel.full_chat.linked_chat_id
        if _ore_cache.get("LINKED_CHATS"):
            _ore_cache["LINKED_CHATS"].update({event.chat_id: {"linked_chat": _ignore}})
        else:
            _ore_cache.update(
