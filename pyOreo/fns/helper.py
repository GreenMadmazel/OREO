...
if run_as_module:
    ...
    async def heroku_logs(event):
        ...
        ok = app.get_log()
        with open("oreo-heroku.log", "w") as log:
            log.write(ok)
        await event.client.send_file(
            event.chat_id,
            file="oreo-heroku.log",
            thumb=OREConfig.thumb,
            caption="**oreo Heroku Logs.**",
        )

        os.remove("oreo-heroku.log")
        await xx.delete()

    async def def_logs(ore, file):
        await ore.respond(
            "**Oreo Logs.**",
            file=file,
            thumb=OREConfig.thumb,
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Oreo {oreo_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Oreo {oreo_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
...
async def restart(ore=None):
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        try:
            Heroku = heroku3.from_key(Var.HEROKU_API)
            app = Heroku.apps()[Var.HEROKU_APP_NAME]
            if ore:
                await ore.edit("`Restarting your app, please wait for a minute!`")
            app.restart()
...
async def shutdown(ore):
    from .. import HOSTED_ON, LOGS

    ore = await eor(ore, "Shutting Down")
    if HOSTED_ON == "heroku":
        if not (Var.HEROKU_APP_NAME and Var.HEROKU_API):
            return await ore.edit("Please Fill `HEROKU_APP_NAME` and `HEROKU_API`")
        dynotype = os.getenv("DYNO").split(".")[0]
        try:
            Heroku = heroku3.from_key(Var.HEROKU_API)
            app = Heroku.apps()[Var.HEROKU_APP_NAME]
            await ore.edit("`Shutting Down your app, please wait for a minute!`")
