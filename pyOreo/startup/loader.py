...
def load_other_plugins(addons=None, pmbot=None, manager=None, vcbot=None):
    ...
    if addons:
        ...
        if not os.path.exists("addons"):
            subprocess.run(
                f"git clone -q -b {Repo().active_branch} https://github.com/itzNightmare17/OREOAddons.git addons",
                shell=True,
            )
        else:
        ...
        if not os.path.exists("addons"):
            subprocess.run(
                "git clone -q https://github.com/itzNightmare17/OREOAddons.git addons",
                shell=True,
            )
