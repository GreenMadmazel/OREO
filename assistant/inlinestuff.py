...
from pyOreo.fns.misc import google_search
...
from . import *
from . import _ore_cache

SUP_BUTTONS = [
    [
        Button.url("• Repo •", url="https://github.com/NightMare17O/Ultroid"),
...
orepic = "https://graph.org/file/4136aa1650bc9d4109cc5.jpg"
...
async def repo(e):
    res = [
        await e.builder.article(
            ...
            thumb=wb(orepic, 0, "image/jpeg", []),
            text="• **OREO USERBOT** •",
            buttons=SUP_BUTTONS,
        ),
    ]
    await e.answer(res, switch_pm="Oreo Repo.", switch_pm_param="start")
...
async def piston_run(event):
    try:
        ...
        result = await event.builder.article(
            ...
            text=f'**Inline Usage**\n\n`@{asst.me.username} run python print("hello world")`\n\n[Language List](https://https://graph.org/Oreo-08-21)',
        )
    ...
    if lang in PISTON_LANGS.keys():
        ...
        result = await event.builder.article(
            ...
            text=f'**Inline Usage**\n\n`@{asst.me.username} run python print("hello world")`\n\n[Language List](https://graph.org/Oreo-08-21)',
        )
        return await event.answer([result])
...
async def twitter_search(event):
    ...
    if _ore_cache.get("twitter"):
        _ore_cache["twitter"].update({match: reso})
    else:
        _ore_cache.update({"twitter": {match: reso}})
...
async def inline_tl(ore):
    try:
        match = ore.text.split(maxsplit=1)[1]
    except IndexError:
        ...
        return await ore.answer(
            [
                await ore.builder.article(
                    title="How to Use?",
                    description="Tl Searcher by Oreo",
    ...
    await ore.answer(res[:50], switch_pm=mo, switch_pm_param="start")
...
InlinePlugin.update(
    {
        ...
        "Sᴇᴀʀᴄʜ Oɴ Gᴏᴏɢʟᴇ": "go TeamOreo",
        "WʜɪSᴘᴇʀ": "wspr @username Hello🎉",
        "YᴏᴜTᴜʙᴇ Dᴏᴡɴʟᴏᴀᴅᴇʀ": "yt Ed Sheeran Perfect",
        "Piston Eval": "run javascript console.log('Hello Oreo')",
