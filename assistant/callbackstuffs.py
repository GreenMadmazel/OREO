...
try:
    from pyOreo.fns.gDrive import GDriveManager
except ImportError:
    GDriveManager = None
...
from pyOreo.fns.helper import fast_download, progress
from pyOreo.fns.tools import Carbon, async_searcher, get_paste, telegraph_client
from pyOreo.startup.loader import Loader
...
_buttons = {
    "otvars": {
        "text": "Other Variables to set for OREO:",
        "buttons": [
            [
...
async def name(event):
    m = range(1, 10)
    tored = [Button.inline(f"{x}", data=f"wrns_{x}") for x in m]
    lst = list(zip(tored[::3], tored[1::3], tored[2::3]))
