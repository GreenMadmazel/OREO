...
from pyOreo.fns.tools import cmd_regex_replace

from . import HNDLR, LOGS, OWNER_NAME, asst, get_string, inline_pic, udB, oreo_cmd
...
@oreo_cmd(pattern="help( (.*)|$)")
async def _help(ore):
    plug = ore.pattern_match.group(1).strip()
    ...
    if plug:
        try:
            ...
            LOGS.exception(er)
            await ore.eor("Error 🤔 occured.")
    else:
        try:
            results = await ore.client.inline_query(asst.me.username, "ored")
            ...
            return await ore.eor(get_string("help_3"))
        await results[0].click(chat.id, reply_to=ore.reply_to_msg_id, hide_via=True)
        await ore.delete()
