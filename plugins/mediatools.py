...
from pyOreo.fns.misc import rotate_image
from pyOreo.fns.tools import make_html_telegraph
...
@oreo_cmd(pattern="rotate( (.*)|$)")
async def rotate_(ore):
    match = ore.pattern_match.group(1).strip()
    if not ore.is_reply:
        return await ore.eor("`Reply to a media...`")
    if match:
        try:
            match = int(match)
        except ValueError:
            match = None
    if not match:
        return await ore.eor("`Please provide a valid angle to rotate media..`")
    ...
    if reply.video:
        ...
        media = await ore.client.download_media(photo or reply)
    ...
    if os.path.exists(file):
        await ore.client.send_file(
            ore.chat_id, file=file, video_note=bool(reply.video_note), reply_to=reply.id
        )
    os.remove(media)
