...
import re
from . import Button, asst, callback, get_string, in_pattern, udB, oreo_cmd
...
tored = [Button.inline(f"{x}", data=f"calc{x}") for x in m]
lst = list(zip(tored[::4], tored[1::4], tored[2::4], tored[3::4]))
lst.append([Button.inline("=", data="calc=")])


@oreo_cmd(pattern="calc")
async def icalc(e):
    udB.del_key("calc")
    if e.client._bot:
