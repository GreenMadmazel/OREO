...
@oreo_cmd(pattern="tr( (.*)|$)", manager=True)
async def _(event):
...
@oreo_cmd(pattern="bots( (.*)|$)", groups_only=True, manager=True)
async def _(ore):
    mentions = "• **Bots in this Chat**: \n"
    if input_str := ore.pattern_match.group(1).strip():
        mentions = f"• **Bots in **{input_str}: \n"
        try:
            chat = await ore.client.parse_id(input_str)
        except Exception as e:
            return await ore.eor(str(e))
    else:
        chat = ore.chat_id
    try:
        async for x in ore.client.iter_participants(
            chat,
...
async def _(ore):
    input_ = ore.pattern_match.group(1).strip()
    if not input_:
        return await ore.eor("`Input some link`", time=5)
    text = None
    ...
    if not text:
        text = "ㅤㅤㅤㅤㅤㅤㅤ"
    await ore.eor(f"[{text}]({input_})", link_preview=False)
...
async def _(e):
    ...
    try:
        ...
        with io.BytesIO(str.encode(text)) as out_file:
            ...
            await e.reply(f"`{e.text}`", file=out_file, thumb=OREConfig.thumb)
        await e.delete()
