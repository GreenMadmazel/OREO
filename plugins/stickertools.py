...
async def pack_kangish(_):
    ...
    _packname = cmdtext or f"Oreo Kang Pack By {_.sender_id}"
    typee = None
    if not local:
    ...
    try:
        short_name = "ore_" + _packname.replace(" ", "_") + str(_.id)
        _r_e_s = await asst(
...
async def hehe(args):
    ...
    if message.photo:
        photo = io.BytesIO()
        photo = await oreo_bot.download_media(message.photo, photo)
    elif message.file and "image" in message.file.mime_type.split("/"):
        photo = io.BytesIO()
        await oreo_bot.download_file(message.media.document, photo)
        if (
        ...
        if (message.file.duration or 0) <= 10:
            ...
            heh, lol = y.read()
            cv2.imwrite("ore.webp", lol)
            photo = "ore.webp"
    elif message.file and "tgsticker" in message.file.mime_type:
        await oreo_bot.download_file(
    ...
    if photo:
        ...
        if not emoji:
            emoji = "🏵"
        if len(splat) == 3:
            pack = splat[2]  # User sent oreo_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        packname = f"ore_{user.id}_{pack}"
        ...
        if (
            ...
            async with oreo_bot.conversation("@Stickers") as conv:
                try:
                    await conv.send_message("/addsticker")
                except YouBlockedUserError:
                    LOGS.info("Unblocking @Stickers for kang...")
                    await oreo_bot(functions.contacts.UnblockRequest("stickers"))
                    await conv.send_message("/addsticker")
                ...
                await conv.get_response()
                await oreo_bot.send_read_acknowledge(conv.chat_id)
        else:
...
async def oredround(event):
    ...
    Image.fromarray(npImage).save("ore.webp")
...
async def oredestroy(event):
    ore = await event.get_reply_message()
    if not (ore and ore.media and "animated" in mediainfo(ore.media)):
        return await event.eor(get_string("sts_2"))
    await event.client.download_media(ore, "oreo.tgs")
    xx = await event.eor(get_string("com_1"))
    await bash("lottie_convert.py oreo.tgs json.json")
    ...
    file = await con.animated_sticker("json.json", "oreo.tgs")
    if file:
        await event.client.send_file(
            event.chat_id,
            file="oreo.tgs",
...
async def oreiny(event):
    ...
    im1 = Image.open("resources/extras/oreo_blank.png")
    if ik.endswith(".tgs"):
        ...
        open("json.json", "w").write(jsn)
        await con.animated_sticker("json.json", "ore.tgs")
        file = "ore.tgs"
        os.remove("json.json")
