...
from pyOreo.dB.filestore_db import del_stored, get_stored_msg, list_all_stored_msgs
from pyOreo.fns.tools import get_file_link

from . import HNDLR, asst, get_string, in_pattern, udB, oreo_bot, oreo_cmd


@oreo_cmd(pattern="store$")
...
async def _(event):
    ...
    del_stored(match[1])
    await oreo_bot.delete_messages(udB.get_key("LOG_CHANNEL"), int(msg_id))
