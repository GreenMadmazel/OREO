...
from pyOreo._misc._assistant import asst_cmd
from pyOreo.dB.gban_mute_db import is_gbanned
from pyOreo.fns.tools import get_chat_and_msgid
...
from pyOreo.fns.info import get_chat_info
...
@oreo_cmd(pattern="kickme$", fullsudo=True)
async def leave(ore):
    await ore.eor(f"`{ore.client.me.first_name} has left this group, bye!!.`")
    await ore.client(LeaveChannelRequest(ore.chat_id))
...
@oreo_cmd(pattern="paste( (.*)|$)", manager=True, allow_all=True)
async def _(event):
    try:
...
async def _(ore):
    ...
    to_add_users = ore.pattern_match.group(1).strip()
    if not ore.is_channel and ore.is_group:
        for user_id in to_add_users.split(" "):
            try:
                await ore.client(
                    AddChatUserRequest(
                        chat_id=ore.chat_id,
                        user_id=await ore.client.parse_id(user_id),
                        fwd_limit=1000000,
                    ),
                )
                await xx.edit(f"Successfully invited `{user_id}` to `{ore.chat_id}`")
        ...
        for user_id in to_add_users.split(" "):
            try:
                await ore.client(
                    InviteToChannelRequest(
                        channel=ore.chat_id,
                        users=[await ore.client.parse_id(user_id)],
                    ),
                )
                await xx.edit(f"Successfully invited `{user_id}` to `{ore.chat_id}`")
            except UserBotError:
...
async def abs_rmbg(event):
    ...
    if zz.mode != "RGB":
        zz.convert("RGB")
    wbn = check_filename("ore-rmbg.webp")
    zz.save(wbn, "webp")
...
async def telegraphcmd(event):
    xx = await event.eor(get_string("com_1"))
    match = event.pattern_match.group(1).strip() or "Oreo"
    reply = await event.get_reply_message()
    if not reply:
...
@oreo_cmd(pattern="suggest( (.*)|$)", manager=True)
