...
from . import bash, get_string, mediainfo, oreo_cmd


@oreo_cmd(pattern="glitch$")
async def _(e):
    try:
    ...
    cmd = f"glitch_me gif --line_count 200 -f 10 -d 50 '{ok}' ore.gif"
    await bash(cmd)
    await e.reply(file="ore.gif", force_document=False)
    await xx.delete()
    os.remove(ok)
    os.remove("ore.gif")
