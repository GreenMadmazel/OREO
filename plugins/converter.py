...
async def _(e):
    ...
    await bash(f"wget {nn} -O resources/extras/oreo.jpg")
...
async def uconverter(event):
    ...
    file = await con.convert(b, outname="oreo", convert_to=convert)
    if file:
...
async def _(event):
    ...
    await event.reply(file=input_str, thumb=OREConfig.thumb)
    await xx.delete()
    os.remove(input_str)
