...
from . import LOGS, asst, eor, events, get_string, udB, oreo_bot, oreo_cmd
...
async def source(e):
    ...
    if not SourceM.contains(y):
        ...
        oreo_bot.add_handler(autopost_func, events.NewMessage())
    else:
        await e.eor(get_string("cha_3"))


@oreo_cmd(pattern="dsource( (.*)|$)")
async def dd(event):
...
async def list_all(event):
    ...
    for channel in channels:
        name = ""
        try:
            name = get_display_name(await oreo_bot.get_entity(int(channel)))
        except BaseException:
            name = ""
        msg += f"\n=> **{name}** [`{channel}`]"
    msg += f"\nTotal {num} channels."
