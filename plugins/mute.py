...
from pyOreo.dB.mute_db import is_muted, mute, unmute
from pyOreo.fns.admins import ban_time

from . import asst, eod, get_string, inline_mention, oreo_bot, oreo_cmd
...
async def startmute(event):
    ...
    if input_ := event.pattern_match.group(1).strip():
        ...
        if reply.out or userid in [oreo_bot.me.id, asst.me.id]:
