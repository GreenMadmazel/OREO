...
import os
from pyOreo.startup.loader import load_addons
from . import LOGS, async_searcher, eod, get_string, safeinstall, oreo_cmd, un_plug


@oreo_cmd(pattern="install", fullsudo=True)
...
@oreo_cmd(pattern="getaddons( (.*)|$)", fullsudo=True)
async def get_the_addons_lol(event):
    ...
    with open(fil, "w", encoding="utf-8") as uore:
        uore.write(plug)
