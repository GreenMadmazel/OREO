...
from . import Carbon, oreo_cmd, get_string, inline_mention
...
async def cr_bn(event):
    ...
    xx = await Carbon(code=code, file_name="oreo_carbon", backgroundColor=col)
...
@oreo_cmd(pattern="rayso")
async def pass_on(ore):
    spli = ore.text.split()
    theme, dark, title, text = None, True, get_display_name(ore.chat), None
    if len(spli) > 2:
        ...
        if spli[1] in RaySoTheme:
            ...
            try:
                text = ore.text.split(maxsplit=1)[1]
            except IndexError:
                pass
    if not theme:
        theme = random.choice(RaySoTheme)
    if ore.is_reply:
        msg = await ore.get_reply_message()
        text = msg.message
