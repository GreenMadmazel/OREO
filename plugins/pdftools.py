...
from pyOreo.fns.tools import four_point_transform
...
async def pdfseimg(event):
    ...
    if not msg:
        ...
        for num in range(pdf.numPages):
            ...
            fil = os.path.join(f"pdf/ore{num + 1}.png")
            ok.append(fil)
        ...
        for o in range(ok):
            ...
            with open(os.path.join("ore.png"), "wb") as f:
                pw.write(f)
            await event.reply(
                file="ore.png",
            )
        ...
        os.remove(pdfp)
        try:
            await event.reply(file="ore.png")
        except PhotoSaveFileInvalidError:
            await event.reply(file="ore.png", force_document=True)
        os.remove("ore.png")
...
async def imgscan(event):
    ...
    if not (
        ...
        return
    oret = await ok.download_media()
    xx = await event.eor(get_string("com_1"))
    image = cv2.imread(oret)
    ...
    scann = f"Scanned {oret.split('.')[0]}.pdf"
    im1.save(scann)
...
async def savepdf(event):
    ...
    if not (ok and (ok.media)):
        ...
        return
    oret = await ok.download_media()
    if oret.endswith(("png", "jpg", "jpeg", "webp")):
        xx = await event.eor(get_string("com_1"))
        image = cv2.imread(oret)
        original_image = image.copy()
        ratio = image.shape[0] / 500.0
