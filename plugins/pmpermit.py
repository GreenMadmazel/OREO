...
if udB.get_key("PMSETTING"):
    if udB.get_key("AUTOAPPROVE"):
        ...
        async def autoappr(e):
            ...
            try:
                await oreo_bot.edit_folder(miss.id, folder=0)
    ...
    async def permitpm(event):
        ...
        if not keym.contains(user.id) and event.text != UND:
            if Redis("MOVE_ARCHIVE"):
                try:
                    await oreo_bot.edit_folder(user.id, folder=1)
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    if inline_pm:
                        results = await oreo_bot.inline_query(
                        ...
                        _to_delete[user.id] = await oreo_bot.send_file(
                            user.id,
                            PMPIC,
                            caption=message_,
                        )
                    else:
                        _to_delete[user.id] = await oreo_bot.send_message(
                            user.id, message_
                        )
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await oreo_bot(BlockRequest(user.id))
                await oreo_bot(ReportSpamRequest(peer=user.id))
                await asst.edit_message(
                    udB.get_key("LOG_CHANNEL"),
                    _not_approved[user.id],
    ...
    @oreo_cmd(pattern="(start|stop|clear)archive$", fullsudo=True)
    async def _(e):
    ...
    @oreo_cmd(pattern="(a|approve)(?: |$)", fullsudo=True)
    ...
    @oreo_cmd(pattern="(da|disapprove)(?: |$)", fullsudo=True)
...
@oreo_cmd(pattern="block( (.*)|$)", fullsudo=True)
...
@oreo_cmd(pattern="unblock( (.*)|$)", fullsudo=True)
...
@oreo_cmd(pattern="listapproved$", owner=True)
async def list_approved(event):
    ...
    for i in all:
        try:
            name = get_display_name(await oreo_bot.get_entity(i))
        except BaseException:
            name = ""
...
async def apr_in(event):
    ...
    if not keym.contains(uid):
        keym.add(uid)
        try:
            await oreo_bot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user = await oreo_bot.get_entity(uid)
        except BaseException:
...
async def blck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await oreo_bot(BlockRequest(uid))
    except BaseException:
        pass
    try:
        user = await oreo_bot.get_entity(uid)
...
async def unblck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await oreo_bot(UnblockRequest(uid))
    except BaseException:
        pass
    try:
        user = await oreo_bot.get_entity(uid)
...
async def ytfuxist(e):
    try:
        ...
        await oreo_bot.delete_messages(e.chat_id, e.id)
...
async def in_pm_ans(event):
    ...
    await event.answer(res, switch_pm="• Oreo •", switch_pm_param="start")


@callback(re.compile("admin_only(.*)"), from_users=[oreo_bot.uid])
