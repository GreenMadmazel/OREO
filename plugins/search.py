...
from pyOreo.fns.tools import get_google_images, saavn_search

from . import LOGS, async_searcher, con, eod, fast_download, get_string, oreo_cmd
...
async def gitsearch(event):
    ...
    ore = await async_searcher(url, re_json=True)
    try:
        uname = ore["login"]
        uid = ore["id"]
        ...
        ucomp = ore["company"]
        ublog = ore["blog"]
        ulocation = ore["location"]
        ubio = ore["bio"]
        urepos = ore["public_repos"]
        ufollowers = ore["followers"]
        ufollowing = ore["following"]
...
@oreo_cmd(pattern="reverse$")
async def reverse(event):
    reply = await event.get_reply_message()
    if not reply:
        return await event.eor("`Reply to an Image`")
    ore = await event.eor(get_string("com_1"))
    ...
    link = alls["href"]
    text = alls.text
    await ore.edit(f"`Dimension ~ {x} : {y}`\nSauce ~ [{text}](google.com{link})")
