...
from pyOreo._misc._assistant import callback, in_pattern
from pyOreo.dB._core import HELP, LIST
from pyOreo.fns.helper import gen_chlog, time_formatter, updater
from pyOreo.fns.misc import split_list
...
async def inline_alive(o):
    TLINK = inline_pic() or "https://graph.org/file/ba0426979cd0dc555c57d.jpg"
    MSG = "• **Oreo Userbot •**"
    WEB0 = InputWebDocument(
        "https://graph.org/file/5e925a0a876a31adde44f.jpg", 0, "image/jpg", []
    )
    RES = [
...
async def help_func(ore):
    key, count = ore.data_match.group(1).decode("utf-8").split("_")
    if key == "VCBot" and HELP.get("VCBot") is None:
        return await ore.answer(get_string("help_12"), alert=True)
    elif key == "Addons" and HELP.get("Addons") is None:
        return await ore.answer(get_string("help_13").format(HNDLR), alert=True)
    if "|" in count:
        _, count = count.split("|")
    count = int(count) if count else 0
    text = _strings.get(key, "").format(OWNER_NAME, len(HELP.get(key)))
    await ore.edit(text, buttons=page_num(count, key), link_preview=False)
