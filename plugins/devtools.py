...
from pyOreo import _ignore_eval
...
try:
    ...
    from pyOreo.fns.tools import safe_load
try:
...
@oreo_cmd(pattern="bash", fullsudo=True, only_devs=True)
async def _(event):
    ...
    if len(OUT) > 4096:
        ored = err + out
        with BytesIO(str.encode(ored)) as out_file:
...
@oreo_cmd(pattern="eval", fullsudo=True, only_devs=True)
async def _(event):
    try:
        cmd = event.text.split(maxsplit=1)[1]
    except IndexError:
    ...
    if any(item in cmd for item in KEEP_SAFE().All) and (
        not (event.out or event.sender_id == oreo_bot.uid)
    ):
...
@oreo_cmd(pattern="cpp", only_devs=True)
async def doie(e):
