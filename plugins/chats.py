...
from . import HNDLR, LOGS, asst, con, get_string, mediainfo, os, types, udB, oreo_cmd
...
async def _(ore):
    ...
    if not ore.client._bot and match:
        try:
            chat = await ore.client.parse_id(match)
        except Exception as ok:
            return await ore.eor(str(ok))
    else:
        chat = ore.chat_id
    ...
    if reply.photo or reply.sticker or reply.video:
        ...
        return await ore.eor("Reply to a Photo or Video..")
    ...
    try:
        ...
        await ore.client(EditPhotoRequest(chat, file))
        await ore.eor("`Group Photo has Successfully Changed !`", time=5)
    except Exception as ex:
        await ore.eor(f"Error occured.\n`{str(ex)}`", time=5)
    os.remove(replfile)
...
async def _(ore):
    ...
    if not ore.client._bot and match:
        chat = match
    try:
        await ore.client(EditPhotoRequest(chat, types.InputChatPhotoEmpty()))
        text = "`Removed Chat Photo..`"
    except Exception as E:
        text = str(E)
    return await ore.eor(text, time=5)


@oreo_cmd(pattern="unbanall$", manager=True, admins_only=True, require="ban_users")
async def _(event):
