...
from . import check_filename, get_string, oreo_bot, oreo_cmd


@oreo_cmd(pattern="qrcode( (.*)|$)")
async def cd(e):
    ...
    default, cimg = OREConfig.thumb, None
    if reply and (reply.sticker or reply.photo):
        cimg = await reply.download_media()
    elif oreo_bot.me.photo and not oreo_bot.me.photo.has_video:
        cimg = await e.client.get_profile_photos(oreo_bot.uid, limit=1)[0]
